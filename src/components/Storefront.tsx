...
export function Storefront({ paymentMode = 'disabled' }: { paymentMode?: PaymentMode | 'disabled' }) {
  ...
  return <>
    ...
    {modal === 'checkout' && <CheckoutDialog paymentMode={paymentMode} items={cart.items} city={city} address={address} onClose={close} onBack={() => setModal('cart')} onConfirm={(value) => { setOrder(value); setCity(value.address.city as City); setAddress(value.address); cart.clear(); setModal('confirmation'); }} />}
