        ...
        <div className="admin-day-content"><div className="admin-day-heading"><div><p className="eyebrow">НЕДЕЛЯ {week}</p><h2>{days[day - 1]}</h2></div><button type="button" className="admin-text-button" onClick={() => setPreview(true)}><Eye size={18} /> Предпросмотр</button></div>
          ...
          {todayMeals.map((meal, index) => <article className="admin-meal" key={meal.id}>
            ...
            <div className="admin-total-nutrition">
              <label className="admin-checkbox"><input type="checkbox" checked={meal.nutritionMode === 'dishes'} onChange={event => editMeal(meal.id, { nutritionMode: event.target.checked ? 'dishes' : 'manual', nutrition: getMealNutrition(meal) })} />Рассчитывать КБЖУ по блюдам</label>
              {meal.nutritionMode === 'dishes' ? <><MealNutrition value={getMealNutrition(meal)} /><p className="admin-nutrition-help">Сумма всех позиций с напитком, без подарочной выпечки. Заполните КБЖУ каждого блюда ниже.</p></> : <NutritionEditor value={meal.nutrition} onChange={nutrition => editMeal(meal.id, { nutrition })} />}
            </div>
