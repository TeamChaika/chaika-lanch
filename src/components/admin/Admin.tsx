...
export function Admin() {
  ...
  return <div className="admin-app">
    <header className="admin-header"><div className="admin-brand"><CalendarDays size={26} /><div><strong>Чайка Обеды</strong><span>Управление меню</span></div></div><div className="admin-header-actions"><a href="/" target="_blank" rel="noreferrer" className="admin-text-button"><ExternalLink size={17} /> Сайт</a><button className="admin-text-button" onClick={() => { setError(''); setPasswordOpen(true); }} disabled={locked}><LockKeyhole size={17} /> Пароль</button><button className="admin-text-button" onClick={signOut} disabled={locked}><LogOut size={17} /> Выйти</button></div></header>
