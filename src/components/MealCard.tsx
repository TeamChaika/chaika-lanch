...
export function MealCard({ meal, quantity, onOpen, onAdd, onRemove, eager = false }: { eager?: boolean; meal: Meal; quantity: number; onOpen: () => void; onAdd: () => void; onRemove: () => void }) {
  return <article className="meal-card">
    <button className="meal-photo-button" type="button" onClick={onOpen} aria-label={`Состав обеда ${meal.name}`}>
      <FoodImage src={meal.image} variants={meal.imageVariants} alt="Пример подачи комплексного обеда Чайка" width={1280} height={853} className="meal-photo" eager={eager} sizes="(max-width: 700px) calc(100vw - 36px), (max-width: 1320px) 46vw, 620px" />
      <span className="meal-tag">{meal.tag}</span>
      <span className="photo-arrow"><ArrowUpRight size={20} /></span>
      <span className="photo-caption">{meal.imageIsExample !== false ? "Пример подачи · состав в меню" : "Фото этого комплекса"}</span>
    </button>
    <div className="meal-card-body">
      <button type="button" className="meal-title" onClick={onOpen}>{meal.name}</button>
      <ul className="meal-composition">{meal.dishes.map((dish, index) => <li key={index}><div><small>{dish.category}</small><span>{dish.name}</span><DishNutrition value={dish.nutrition} /></div><span className="dish-weight">{dish.weight} г</span></li>)}</ul>
