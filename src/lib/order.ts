import { defaultCatalog, type MenuCatalog, findMeal, isMealAvailable, menuWeekForDate } from '@/data/menu';
...
export function deliveryWeeks(now = new Date(), catalog: MenuCatalog = defaultCatalog): DeliveryWeek[] {
  const localDay = new Intl.DateTimeFormat('sv-SE', { timeZone: 'Europe/Simferopol' }).format(now);
  const first = new Date(`${localDay < catalog.cycleStartsOn ? catalog.cycleStartsOn : localDay}T12:00:00Z`);
  ...
  return Array.from({ length: 4 }, (_, weekIndex) => {
    ...
    return { value, menuWeek: menuWeekForDate(value, catalog)!, label: `${formatDate(value)} — ${formatDate(days[4].value)}`, days: days.filter((day) => day.value >= localDay) };
  });
}

export const deliveryDays = (now = new Date(), catalog: MenuCatalog = defaultCatalog): DeliveryDay[] => deliveryWeeks(now, catalog).flatMap((week) => week.days);
...
export const cartTotal = (items: CartItem[], catalog: MenuCatalog = defaultCatalog) => items.reduce((sum, item) => sum + (findMeal(item.mealId, catalog)?.price ?? 0) * item.quantity, 0);
...
export function addItem(items: CartItem[], mealId: string, date: string, catalog: MenuCatalog = defaultCatalog): CartItem[] {
  if (!isMealAvailable(mealId, date, catalog)) return items;
...
export function readSavedCart(raw: string | null, allowedDays: string[], catalog: MenuCatalog = defaultCatalog): CartItem[] {
  if (!raw) return [];
  const parsed: unknown = JSON.parse(raw);
  if (!Array.isArray(parsed)) return [];
  const valid = parsed.filter((item): item is CartItem => typeof item === 'object' && item !== null && typeof item.mealId === 'string' && typeof item.date === 'string' && allowedDays.includes(item.date) && isMealAvailable(item.mealId, item.date, catalog) && Number.isInteger(item.quantity) && item.quantity > 0 && item.quantity <= 99);
